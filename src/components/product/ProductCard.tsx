...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link href={`/products/${product.id}`} className="flex flex-col h-full w-full">
      <Card className="group overflow-hidden border-0 hover:opacity-90 transition-opacity cursor-pointer flex-grow flex flex-col h-full w-full">
        <div className="w-full relative bg-gray-100 overflow-hidden" style={{ aspectRatio: '3/4' }}>
          {mainImage ? (
            <Image
              src={mainImage}
              alt={product.name}
