...
// --- MAIN IMPORT ---
async function importV5() {
    console.log('--- Starting Import V5 (Clean Slate + Gallery + Variants) ---');
    ...
    console.log('Clearing old data (variants, product_images, products)...');
    ...
    for (let i = 0; i < groupKeys.length; i++) {
        ...
        if (mediaRow) {
            const galleryImages = [];
            // "Hình ảnh sản phẩm 1" is typically same as Cover. Check 1..8
            for (let k = 1; k <= 8; k++) {
                const imgCol = `Hình ảnh sản phẩm ${k}`;
                const url = mediaRow[imgCol];
                if (url) {
                    // Check duplicate
                    if (!galleryImages.includes(url) && url !== mainImage) {
                        galleryImages.push(url);
                    }
                }
            }
            // Also add if mainImage is not in header list? Usually image 1 is cover.
            // If galleryImages empty, take mainImage
            if (galleryImages.length === 0 && mainImage) galleryImages.push(mainImage);

            const imageInserts = galleryImages.slice(0, 5).map((url, idx) => ({
    ...
    console.log(`--- Import V5 Complete. Success: ${successCount} ---`);
}
